...
?>
        ...
        <div class="wrapper">
            ...
            <div class="content-wrapper">
                <section class="content-header">
                    ...
                    <a type='button' id="passou" class='btn btn-danger' href='excluir_evento.php?operacao=excluirPassados'>Excluir eventos antigos</a>
                    <ol class="breadcrumb">
                        <li><a href="#"><i class="fa fa-dashboard"></i> Calendário</a></li>
                    </ol>
                        ...
                        <div class="col-md-3">
                            <div class="box box-solid">
                                <div class="box-header with-border">
                                    <h4 class="box-title">Últimos eventos cadastrados</h4>
                                </div>
                                <div class="box-body">
                                    <!-- the events -->
                                    <div id='external-events'>
                                        ...
                                        $lista = $eventoDAO->listarEventos();
                                        
                                        foreach ($lista as $evento){
                                            echo "<a type='button' id='excluir' class='btn btn-danger btn-md' href='excluir_evento.php?operacao=excluir&id={$evento->getId()}'><i class='fa fa-trash'></i></a><div class='external-event bg-green'>{$evento->getTitle()}</div>";
                                        }
                                        /*
                                        if ($lista[0] != null && $lista[0] != ''){
                                        echo "<a type='button' id='excluir' class='btn btn-danger btn-md' href='excluir_evento.php?operacao=excluir&id={$lista[0]->getId()}'><i class='fa fa-trash'></i></a><div class='external-event bg-green'>{$lista[0]->getTitle()}</div>";
                                        }
                                        if ($lista[1] != null && $lista[1] != ''){
                                        echo "<a type='button' id='excluir' class='btn btn-danger btn-md' href='excluir_evento.php?operacao=excluir&id={$lista[1]->getId()}'><i class='fa fa-trash'></i></a><div class='external-event bg-yellow'>{$lista[1]->getTitle()}</div>";
                                        }
                                        if ($lista[2] != null && $lista[2] != ''){
                                        echo "<a type='button' id='excluir' class='btn btn-danger btn-md' href='excluir_evento.php?operacao=excluir&id={$lista[2]->getId()}'><i class='fa fa-trash'></i></a><div class='external-event bg-aqua'>{$lista[2]->getTitle()}</div>";
                                        }
                                        if ($lista[3] != null && $lista[3] != ''){
                                        echo "<a type='button' id='excluir' class='btn btn-danger btn-md' href='excluir_evento.php?operacao=excluir&id={$lista[3]->getId()}'><i class='fa fa-trash'></i></a><div class='external-event bg-light-blue'>{$lista[3]->getTitle()}</div>";
                                        }
                                        if ($lista[4] != null && $lista[4] != ''){
                                        echo "<a type='button' id='excluir' class='btn btn-danger btn-md' href='excluir_evento.php?operacao=excluir&id={$lista[4]->getId()}'><i class='fa fa-trash'></i></a><div class='external-event bg-red'>{$lista[4]->getTitle()}</div>";
                                        }*/
                            ...
                            <div class="box box-solid">
                                ...
                                <div class="box-body">
                                    <form id="novo_evento" action="" method="post">
                                        ...
                                        <input id="nome" name="nome" type="text" class="form-control" placeholder="Título evento">
                                        <label for="">Data</label>
                                        <input id="data" name='data' type="date" class="form-control">
                                        <div id="espaco">
                                            <button id="" type="reset" class="btn btn-danger">Cancelar</button>
    ...
    <div id='calendario'>
        <br/>
        <form id="novo_evento" action="" method="post">
            Nome do Evento: <input type="text" name="nome" required/><br/><br/> Data do Evento: <input type="date" name="data" required/><br/><br/>
